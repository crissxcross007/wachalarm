...
const homeCenter = [52.12, 13.62];
const homeZoom = window.innerWidth < 700 ? 7.55 : 7.75;
...
function sortChannels(items) {
  return [...items].sort((a, b) => {
    ...
    return normalizeText(`${a.ort} ${a.name}`).localeCompare(
      normalizeText(`${b.ort} ${b.name}`),
      "de"
    );
  });
}

function findMainChannelForRegion(regionName) {
  return sortChannels(kanalOrte).find(item =>
    item.landkreis === regionName && channelGroup(item) === "main"
  );
}

function openMainChannelForRegion(regionName) {
  const mainChannel = findMainChannelForRegion(regionName);
  if (!mainChannel) return;

  focusedItemKey = itemKey(mainChannel);
...
function visibleItems() {
  ...
  return sortChannels(
    kanalOrte.filter(item =>
      item.landkreis === activeRegion &&
      typeMatches(item) &&
      searchMatches(item)
    )
  );
...
function findBestSearchHit(term) {
  ...
  const scored = kanalOrte
    ...
    .sort((a, b) => {
      ...
      return channelPriority(a.item) - channelPriority(b.item);
    });
...
function renderMarkers() {
  ...
  visibleItems().forEach(item => {
    const isMain = channelGroup(item) === "main";
    ...
    if (isMain) {
      marker.bindPopup(`
        <div class="popup-main-channel-v11">
          <span class="tagline-v4 main-badge-v11">Hauptkanal für ${item.landkreis}</span>
          <div class="popup-title-v4">${item.name}</div>
          <div class="popup-meta-v4">
            Alle verfügbaren Einsatzinfos für <strong>${item.landkreis}</strong> gebündelt in einem WhatsApp-Kanal.
          </div>
          ${link}
        </div>
      `);
...
function resetOverviewView() {
  // Bewusst etwas näher als ein kompletter Deutschland-Ausschnitt:
